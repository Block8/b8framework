...
{loop table.relationships.toOne}
    ...
    public function get{@item.php_name}()
    {
        ...
        if (empty($rtn)) {
            $rtn    = Factory::getStore('{@item.table_php_name}')->getBy{@item.col_php}($key);
...
{loop table.relationships.toMany}
    ...
    public function get{@item.php_name}()
    {
        return Factory::getStore('{@item.table_php}')->getBy{@item.from_col_php}($this->get{@item.col_php}());
    }
...
{/loop}
    ...
    {
        return Factory::getStore('{@table.php_name}')->getByPrimaryKey($value, $useConnection);
    }

{loop table.columns}
{if item.unique_indexed}

    public static function getBy{@item.php_name}($value, $useConnection = 'read')
    {
        return Factory::getStore('{@table.php_name}')->getBy{@item.php_name}($value, $useConnection);
    }
{/if}
{if item.many_indexed}

    public static function getBy{@item.php_name}($value, $limit = null, $useConnection = 'read')
    {
        return Factory::getStore('{@table.php_name}')->getBy{@item.php_name}($value, $limit, $useConnection);
    }
{/if}
{/loop}
